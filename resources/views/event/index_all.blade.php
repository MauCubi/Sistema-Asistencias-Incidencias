...
<table class="table table-bordered table-hover table-responsive-sm shadow table-sm">
    ...
    <tbody>
        @if($events->count() == 0)
            <td style="background-color:gainsboro" colspan="6">No hay incidencias registradas</td>
        @else
       ...
       <tr>
            ...
            <td>
                ...
                <a href="{{ route('event.show_all', $event->id)}}" class="btn btn-primary mr-2 btn-sm"><i class="fa fw fa-info"></i> Ver</a>                
                </div>
            </td>
        </tr>
        @endforeach
